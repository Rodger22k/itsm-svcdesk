...
The service is implemented in Python 3.13 with FastAPI. It exposes the required JSON API on port 8080 and uses SQLite in a Docker named volume so tickets survive a container restart. Docker Compose builds the service from the repository and uses no bind mounts or runtime network access.
...
- `src/main.py` contains the FastAPI application, SQLite storage, routes, validation, state transitions and SLA calculations.
- `requirements.txt`, `Dockerfile` and `docker-compose.yml` install dependencies during the image build and start the `svcdesk` service.
...
5. Implement the request test clock using `SVCDESK_TEST_CLOCK` and `X-Test-Clock`.
6. Implement the state transitions `ack`, `start`, `resolve`, `close` and `reopen`, including invalid-transition HTTP 409 responses.
